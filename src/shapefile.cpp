...
void ShapeFile::init(){
    ...
    boundBoxMax = QVector2D(padMaxBound[0], padMaxBound[1]);
    boundBoxMin = QVector2D(padMinBound[0], padMinBound[1]);
    ...
    for (int i = 0; i < nEntities; i++)
    {
        ...
        if (shpType == SHPT_POINT || shpType == SHPT_POINTZ){
            Entity e;
            e.points.push_back(QVector3D(psShape->padfX[0], psShape->padfY[0], psShape->padfZ[0]));
            ...
            for (int i = psShape->nParts - 1; i >= 0; i--)
            {
                ...
                for (; idx >= psShape->panPartStart[i]; idx--)
                {
                    ...
                    e.points.push_back(QVector3D(psShape->padfX[idx], psShape->padfY[idx], psShape->padfZ[idx]));
                }
                entities.push_back(e);
            }
...
QVector4D ShapeFile::getBoundaries(){
    return QVector4D(boundBoxMin[0], boundBoxMin[1], boundBoxMax[0], boundBoxMax[1]);
}

vector<Entity> *ShapeFile::getEntity()
{
    return &this->entities;
}

int ShapeFile::getnEntities()
{
    return nEntities;
}

int ShapeFile::getshpType()
{
    return shpType;
}
